...
│
├── doc/
│   └── issuehub.txt              # hand-written vimdoc reference
├── spec/                         # busted specs
...
└── .github/workflows/
    ...
    └── release.yml               # luarocks-tag-release
```
...
- Release: `luarocks-tag-release` on tag.
- Docs: `doc/issuehub.txt` is **hand-written**, not generated. panvimdoc was the
  original plan; it was dropped because it renders the README, and the README is
  a guide while the help file needs to be a reference. Generating one from the
  other would have produced a worse version of both. A spec asserts that every
  `|tag|` referenced in the help file resolves and that no line exceeds 78
  display columns.
